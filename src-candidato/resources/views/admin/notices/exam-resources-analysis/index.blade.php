<x-manager.app-layout-widescreen>
    <x-slot name="header">
        <x-manager.header tip="Gerenciar Solicitações de Recurso realização de prova" :notice="$notice" :nomeEdital="$notice->number">
            <x-manager.internal-navbar-itens home="admin.notices.exam-resources-analysis.index" :routeVars="['notice'=>$notice]"
                ...
                <table class="min-w-full leading-normal">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="bg-gray-100">{{ $subscription->user->name }}</td>
                            <td>{{ $subscription->user->cpf }}</td>
                            <td class="bg-gray-100">{{ $subscription->user->email }}</td>
                            <td>{{ $subscription->distributionOfVacancy->offer->courseCampusOffer->campus->name }}</td>
                            <td class="bg-gray-100">{{ $subscription->distributionOfVacancy->offer->courseCampusOffer->course->name }}</td>                            
                            <td>{{ $subscription->examResource->description }}</td>    
                            <td class="bg-gray-100">{{ $subscription->exam_resource_description }}</td>  
                            <td>{{ $subscription->exam_resource_analysis['approved_ptBR'] ?? null }}</td>
                            <td>                   
                                <button type="button"
                                    ...
                                    data-recourse="{{ $subscription->getAttributes()['exam_resource_analysis'] }}"                                    
                                    >
                                    Feedback
                                </button>                                                                
                            </td>
                ...
                <div
                    ...
                    <div class="modal-content py-4 text-left z-10 px-6 overflow-y-auto max-h-screen">
                        ...
                        <div class="mt-5">                            
                            ...
                            <h3 class="text-red-700 font-bold">Recurso Necessário</h3>
                            <p x-text="examResource"></p>
                            <p x-text="examDescription"></p>
                        </div>
        ...
        function recourse(){            
            return {
                ...
                openModal: function(event) {                    
                    ...
                    this.examResource = event.target.parentNode.parentNode.children[5].textContent;
                    this.examDescription = event.target.parentNode.parentNode.children[6].textContent;
                ...
                sendForm: function() {         
                    ...
                    axios.put("./exam-resources-analysis/"+this.subscription,{feedback: this.feedback, approved: this.approved})
                        .then((response)=>{        
