<x-manager.app-layout>
    <x-slot name="header">
        <x-manager.header tip="Gerenciar Recursos Recebidos" :notice="$notice" :nomeEdital="$notice->number">
            <x-manager.internal-navbar-itens home="admin.notices.recourses.index" :routeVars="['notice'=>$notice]"
            ...
            <div class="inline-block min-w-full  overflow-hidden">
                ...
                <table class="min-w-full leading-normal">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-5 py-3">{{ $subscription->getSubscriptionNumber()  }} </td>
                            <td class="px-5 py-3">{{ $subscription->user->name }}</td>
                            <td class="px-5 py-3">{{ $subscription->user->cpf }}</td>
                            <td class="px-5 py-3">{{ $subscription->distributionOfVacancy->offer->courseCampusOffer->campus->name }}</td>
                            <td class="px-5 py-3">{{ $subscription->distributionOfVacancy->offer->courseCampusOffer->course->name }}</td>
                            <td class="px-5 py-3">{{ $subscription->distributionOfVacancy->affirmativeAction->slug }}</td>                            
                            <td class="px-5 py-3">{{ $subscription->preliminary_classification_recourse['feedback']['approved_ptBR'] ?? null }}</td>
                            <td>                   
                                <button type="button"
                                    ...
                                    data-recourse="{{ $subscription->getAttributes()['preliminary_classification_recourse'] }}"                                    
                                    >
                                    Feedback
                                </button>                                                                
                            </td>
            ...
            <div x-show="open" id="modal" x-bind:class="{'opacity-0 pointer-events-none': open === false}"
                ...
                <div
                    ...
                    <div class="modal-content py-4 text-left z-10 px-6 overflow-y-auto max-h-screen">
                        ...
                        <!--Body-->
                        <div class="mt-5">
                            <h3 class="text-red-700 font-bold">Justificativa:</h3>
                            <p x-text="recourse.justify"></p>
                            <h3 class="text-red-700 font-bold">Posição Informada:</h3>
                            <p x-text="recourse.position"></p>
                        ...
                        <form id="recourse-feedback-form" class="border-gray-500 border-t-2">                            
                            ...
                            <x-textarea value="" class="w-full" rows="5" x-model="feedback.text"></x-textarea>
                        ...
                        <div class="flex justify-end pt-2">
                            <x-jet-button  x-show="recourse.feedback == null"  type="submit" x-on:click.prevent="sendForm" form="recourse-feedback-form">Salvar</x-jet-button>
                            <p  x-show="recourse.feedback != null" class="bg-red-300 text-red-700 p-5" > Você já emitiu um feedback para esse recurso, não pode ser alterado </p>
        ...
        function recourse(){            
            return {
                ...
                openModal: function(event) {                    
                    ...
                    this.feedback.text = this.recourse.feedback.feedback ?? null;
                    this.feedback.approved = this.recourse.feedback.approved ?? null;
                    this.target = event.target.parentNode.parentNode.children[6];
                },
                sendForm: function() {                    
                    axios.put("./recourses/"+this.subscription,{recourse: this.recourse,feedback: this.feedback})
                        .then((response)=>{        
